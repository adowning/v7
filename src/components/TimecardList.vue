<template>
  <div>
    ...
    >
      ...
      <template v-slot:item.date="{ item }">{{ item.attributes.clockIn | moment("ddd MM") }}</template>
      <template v-slot:item.employee="{ item }">{{ item.employee.attributes.username }}</template>
      <template v-slot:item.clockIn="{ item }">{{ item.attributes.clockIn | moment("HH:MM") }}</template>

      <template v-slot:item.clockOut="{ item }">{{ item.attributes.clockOut | moment("HH:MM") }}</template>

      <template v-slot:expanded-item="{ item }">
        <td
          :colspan="headers.length"
        >clock in location: {{ item.attributes.clockInLocation }} out location: {{ item.attributes.clockOutLocation }} device: {{ item.attributes.device }} method: {{ item.attributes.method }}</td>
      </template>
    </v-data-table>
  </div>
</template>
  ...
  data() {
    return {
      headers: [
        ...
        { text: "Employee", align: "left", sortable: false, value: "user" },
        {
          text: "Clock In",
          align: "center",
          sortable: false,
