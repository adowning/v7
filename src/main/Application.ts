...
const AUTO_CHECK_UPDATE_INTERVAL = 7 * 24 * 60 * 60 * 1000
...
export default class Application extends EventEmitter {
	...
	init() {
		...
		// logger.warn(this.partition)
		this.menuManager = new MenuManager(null)
	...
	start(page, options = {}) {
		logger.warn('[Andrews] page')
		this.showPage(page, options)
	}

	showPage(page, options = {}) {
		logger.warn('asdf', page)
	...
	// 	logger.warn('made a manager ', this.protocolManager)
	// }
	...
	// 	logger.warn('hai')
	// 	this.show()
	// 	logger.warn(this.protocolManager)
	// 	this.protocolManager.handle('tel')
	// }
	...
	handleUpdaterEvents() {
		this.updateManager.on('checking', event => {
			console.log('handleUpdaterEvents:checking===>', event)
		...
		this.updateManager.on('update-not-available', event => {
			console.log('handleUpdaterEvents:update-not-available===>', event)
		...
		this.updateManager.on('will-updated', event => {
			console.log('handleUpdaterEvents:will-updated===>', event)

			this.windowManager.setWillQuit(true)
		})

		this.updateManager.on('update-error', event => {
			console.log('handleUpdaterEvents:update-error===>', event)
			this.menuManager.updateMenuItemEnabledState('app.check-for-updates', true)
		})
	}
	...
	handleCommands() {
		...
		this.on('application:open-file', event => {
			console.log('application:open-file===>', event)
